...
from notes.notes import alter_note, get_note_list, search_note_by_name, deep_search_for_text, delete_note
...
class TestStringMethods(unittest.TestCase):
    ...
    def assertNote(self, orig: Note, expected: Note):
        if orig.path != expected.path:
            ...
            self.fail("{} Does not match {}".format(orig.extra_info, expected.extra_info))


    def test_alter(self):
        alter_note([TEST_NAME, TEST_MSG])
        alter_note([TEST_NAME2, TEST_MSG2])
        self.assertTrue(os.path.exists(os.path.join(self.test_dir, f"{TEST_NAME}.{config.get('extension')}")))
    ...
    def test_x_deletion(self):
        ...
        self.assertFalse(os.path.exists(os.path.join(self.test_dir, f"{TEST_NAME}.{config.get('extension')}")))
